...
function setup_photochemical_equilibrium()
    ...
    for (i, sp) in enumerate(active_longlived)
        active_longlived_species_rates[i, :] .= getrate(sp, sepvecs=true; chemistry_on=do_chem, transport_on=do_trans, chemnet=reaction_network, transportnet, chem_species, transport_species, )
    end
...
# times to save for cycling (seasonal_cycle should be false for Venus)
