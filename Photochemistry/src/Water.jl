...
#                                                                              #
# **************************************************************************** #
# REQUIRES UPDATES FOR VENUS - PROBABLY DON'T SETUP WATER PROFILE 
...
function venus_water_init(atmdict, m; globvars...)
    #=
    Initialize the water profile as a constant mixing ratio m of the total.
    =#

    GV = values(globvars)
    @assert all(x->x in keys(GV), [:all_species, :DH, :n_alt_index])

    atmdict[:H2O] = m .* n_tot(atmdict; GV.n_alt_index, GV.all_species)
