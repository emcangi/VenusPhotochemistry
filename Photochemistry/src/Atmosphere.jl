...
#                                                                              #
# **************************************************************************** #
# Checked for Venus adaptation 20-Dec-22
...
function electron_density(atmdict; ne=1e5, globvars...)
    #=
    Calculate the electron profile for the current atmospheric state. Usually used in 
    set up.
    Inputs:
        atmdict: Current atmospheric state
        ne: Number density of electrons to use for the constant profile type.
    Outputs:
    ...
    if GV.e_profile_type=="constant"
        E = [ne for i in GV.non_bdy_layers]
...
function compile_ncur_all(n_long; n_short=nothing, n_inactive=nothing, globvars...) 
    #=
    ...
    if n_short!=nothing
        ...
        n_cur_active_short = Dict()
    end
    if n_inactive!=nothing
...
function scaleH(z, sp::Symbol, T; globvars...)
    ...
    return @. kB*T/(GV.molmass[sp]*mH*M_P*bigG)*(((z+R_P))^2)
end

function scaleH_lowerboundary(z::Float64, T::Float64; ntot=ntot_at_lowerbdy, globvars...)
    #= 
    Input:
    ...
    mm = GV.molmass[:CO2]
    return kB*T/(mm*mH*M_P*bigG)*(((z+R_P))^2)
end 
...
function scaleH(atmdict::Dict{Symbol, Vector{ftype_ncur}}, T::Vector; globvars...)
    ...
    mm_vec = meanmass(atmdict; globvars...)
    return @. kB*T/(mm_vec*mH*M_P*bigG)*(((GV.alt+R_P))^2)
end
