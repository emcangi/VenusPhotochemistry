...
function boundaryconditions(fluxcoef_dict, atmdict, M; nonthermal=true, globvars...)
    ...
    # assume a zero boundary condition for all species, and we will update that according to the speciesbclist dictionary.
    bc_dict = Dict{Symbol, Array{ftype_ncur}}([s=>[0 0; 0 0] for s in GV.all_species])

    for sp in intersect(keys(GV.speciesbclist), GV.all_species) # prevents error if some species are temporarily turned off, e.g. when trying to converge atm.
        try 
...
function effusion_velocity(Texo, m; globvars...)
    ...
    lambda = (m*mH*bigG*M_P)/(kB*Texo*(R_P+GV.zmax))
...
function escape_probability(sp, atmdict; globvars...)::Array
    ...
    # UPDATED FOR VENUS per Bethan (1/31/23)
    A = 0.868 # escape probability at altitude where above column = 0, for high energy particles. upper limit
    a = 0.058 # how "transparent" the atmosphere is to an escaping atom. smaller for higher energy so this is for an upper limit.
...
function final_escape(thefolder, thefile; globvars...)
    ...
    escdf = DataFrame("EscapeType"=>["Thermal", "Nonthermal", "Total"], 
                      "H"=>[0, 0, 0], "D"=>[0, 0, 0], "H2"=>[0, 0, 0], "HD"=>[0, 0, 0], "Hpl"=>[0, 0, 0], "H2pl"=>[0, 0, 0])

    # Now collect non-thermal and thermal fluxes for each species. 
    for s in ["H", "D", "H2", "HD", "Hpl", "H2pl"]
    ...
    # Calculate total H atoms lost through any species
    escdf."TotalNeutralHAtomsLost" = sum(eachcol(escdf[!, Not([:EscapeType, :H2, :D])])) .+ 2 .* escdf[:, :H2]
    # Calculate total H atoms lost through any species
    escdf."TotalNeutralDAtomsLost" = sum(eachcol(escdf[!, Not([:EscapeType, :H2, :H])])) # adds up 1 * D and 1 * HD
    # Calculate total atoms lost through any species
    escdf."TotalNeutralAtomsLost" = sum(eachcol(escdf[!, Not([:EscapeType, :H2, :HD])])) .+ 2 .* escdf[:, :H2] .+ 2 .* escdf[:, :HD]
